...
class FeedItem extends HtmlDescribable {
	...
	// Added by Peter Rukavina, peter@rukavina.net

	var $categories = Array();

	function addCategory($name,$shortform)
	{
		$this->categories[] = $name;
		if (strtolower($name) <> strtolower($shortform)) {
			$this->categories[] = $shortform;
		}
	}

	// end add, Peter Rukavina
	...
	function createFeed() {
		...
		for ($i=0;$i<count($this->items);$i++) {
			...
			*/
			...
			if (count($this->items[$i]->categories)) {
				foreach($this->items[$i]->categories as $categoryname)
				{
					$feed.= "            <category>".htmlspecialchars($categoryname)."</category>\n";
				}
			}
			...
			if (count($this->items[$i]->enclosures)) {
				...
				{
					$feed.= "            <enclosure url=\"" . $enc['url'] . "\" length=\"" . $enc['length'] . "\" type=\"" . $enc['type'] . "\" />\n";
					if (($this->items[$i]->enclosureFile <> "") and ($enc['type'] == "audio/mpeg")) {
//						$command = "/usr/bin/mp3info -q " . $this->items[$i]->enclosureFile . " | /usr/bin/head -n2";
